...
function proxyAgents() {
  if (!PROXY_URL) return {};
  return { httpAgent: new HttpProxyAgent(PROXY_URL), httpsAgent: new HttpsProxyAgent(PROXY_URL) };
}
// Use proxy for external requests that get blocked
function pAxios(config) { return axios({ ...config, ...proxyAgents() }); }
