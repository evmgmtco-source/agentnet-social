...
async function getKickClips(slug, name) {
  ...
  for (const url of urls) {
    try {
      ...
      addLog(`[Kick] ${name} HTTP ${res.status} — ${JSON.stringify(res.data).slice(0,120)}`);
      const list = res.data?.data || res.data?.clips || res.data || [];
      if (Array.isArray(list) && list.length > 0) {
        const clips = list
          ...
          .filter(c => c.id && c.url);
        addLog(`[Kick] ${name}: ${clips.length} usable clips`);
        return clips;
      } else { addLog(`[Kick] ${name}: empty response`); }
    } catch(e) { addLog(`[Kick] ${name} error: ${e.response?.status||''} ${e.message.slice(0,60)}`); }
  }
  return [];
}
