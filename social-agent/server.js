...
function pAxios(config) {
  if (!PROXY_URL) return axios(config);
  try {
    const httpsAgent = new HttpsProxyAgent(PROXY_URL, { rejectUnauthorized: false });
    const httpAgent  = new HttpProxyAgent(PROXY_URL);
    return axios({ ...config, httpsAgent, httpAgent, proxy: false });
  } catch(e) { return axios(config); }
}
...
// Download yt-dlp binary at startup if not present
function ensureYtDlp() {
  const paths = ['./yt-dlp', '/usr/local/bin/yt-dlp', '/usr/bin/yt-dlp'];
  for (const p of paths) { try { require('child_process').execSync(`${p} --version 2>&1`); return p; } catch(e){} }
  // Not found — download it
  addLog('[Setup] Downloading yt-dlp...');
  try {
    require('child_process').execSync('curl -sL https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp -o ./yt-dlp && chmod +x ./yt-dlp', {timeout:30000});
    addLog('[Setup] yt-dlp downloaded ✓');
    return './yt-dlp';
  } catch(e) { addLog('[Setup] yt-dlp download failed: '+e.message); return 'yt-dlp'; }
}
let YTDLP = './yt-dlp'; // will be set after ensureYtDlp()
function checkTools() {
  const r={};
...
// ── YOUTUBE AUTO-DISCOVERY (no proxy needed — YouTube doesn't block servers) ───
const YT_QUERIES = [
  'xQc kick stream highlights today',
  'AdinRoss kick clip funny',
  'IShowSpeed kick moments',
  'KaiCenat kick stream clip',
  'Trainwreck kick highlights',
  'kick.com best clips this week',
  'kick streamer funny moments viral',
];

function searchYouTube(query) {
  return new Promise(resolve => {
    const cmd = `${YTDLP} --flat-playlist --dump-json --playlist-end 3 --no-warnings "ytsearch3:${query}" 2>&1`;
    exec(cmd, { timeout:45000 }, (err, stdout) => {
      const lines = (stdout||'').split('\n').filter(l=>l.trim().startsWith('{'));
      const results = lines
        .map(l=>{try{return JSON.parse(l);}catch{return null;}})
        .filter(Boolean)
        .filter(c=>c.id && (c.duration||0)>=10 && (c.duration||0)<=180)
        .map(c=>({ id:'yt_'+c.id, title:c.title||query, url:`https://www.youtube.com/watch?v=${c.id}`, views:c.view_count||0, duration:c.duration||30, streamerName:extractStreamer(c.title||''), source:'youtube' }));
      addLog(`[YouTube] "${query.slice(0,35)}": ${results.length} results`);
      resolve(results);
    });
  });
}

function extractStreamer(title) {
  const names=['xQc','AdinRoss','Trainwreck','KaiCenat','IShowSpeed','Speed','N3on','Jynxzi','Nickmercs'];
  for (const n of names){if(title.toLowerCase().includes(n.toLowerCase()))return n;}
  return title.split(/[|\-–]/)[0].trim().slice(0,20)||'Kick';
}
...
async function autoDiscover() {
  addLog('[Auto] Searching YouTube for Kick clips...');
  const db = loadDB();
  const postedIds = db.posted.map(p=>p.id||p.clipId);

  // Rotate queries each run for variety
  const offset = db.posted.length % YT_QUERIES.length;
  const queries = [...YT_QUERIES.slice(offset), ...YT_QUERIES.slice(0,offset)].slice(0,4);

  let allClips = [];
  for (const q of queries) {
    const clips = await searchYouTube(q);
    allClips.push(...clips);
  ...
  const newClips = allClips.filter(c=>!postedIds.includes(c.id)).sort((a,b)=>b.views-a.views).slice(0,2);
