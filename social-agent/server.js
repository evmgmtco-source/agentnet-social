...
function getClipsForStreamer(slug, name) {
  return new Promise(resolve => {
    const cmd = `yt-dlp --flat-playlist --dump-json --playlist-end 8 --no-warnings "https://kick.com/${slug}/clips" 2>&1`;
    exec(cmd, { timeout: 45000 }, (err, stdout) => {
      const lines = (stdout||'').split('\n').filter(l => l.trim().startsWith('{'));
      if (lines.length === 0) {
        addLog(`[Kick] ${name}: 0 clips (yt-dlp: ${(err&&err.message||'no output').slice(0,60)})`);
        resolve([]); return;
      }
      const clips = lines
        ...
        .filter(c => c.id);
      addLog(`[Kick] ${name}: ${clips.length} clips via yt-dlp`);
      resolve(clips);
    });
  });
}
...
function downloadClip(url, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    exec(cmd, { timeout: 90000 }, async (err, stdout) => {
      ...
      addLog(`[Download] yt-dlp failed, trying direct download...`);
      try {
        ...
        w.on('error', reject);
      } catch(e2) { reject(new Error('Both download methods failed: '+e2.message)); }
    });
  });
}
